...
heatmap.function <- function(x,
                             ...
                             na.rm = TRUE,
                             revC = identical(cols_value, "row_value"),
                             add.expr,
                             breaks,
  ...
  if (is.null(row_value) || is.na(row_value))
    row_value <- FALSE
  if (is.null(cols_value) || is.na(cols_value))
    cols_value <- FALSE
  else if (cols_value == "row_value" && !isTRUE(row_value))
    cols_value <- FALSE
  ...
  if (!inherits(row_value, "dendrogram")) {
    if (((!isTRUE(row_value)) || (is.null(row_value))) && (dendrogram %in%
                                                 c("both", "row"))) {
      if (is.logical(cols_value) && (cols_value))
        dendrogram <- "column"
      else
        dedrogram <- "none"
      warning(
        "Discrepancy: row_value is FALSE, while dendrogram is `",
        dendrogram,
        "'. Omitting row dendogram."
      )
    }
  }
  if (!inherits(cols_value, "dendrogram")) {
    if (((!isTRUE(cols_value)) || (is.null(cols_value))) && (dendrogram %in%
                                                 c("both", "column"))) {
      if (is.logical(row_value) && (row_value))
        dendrogram <- "row"
      else
        dendrogram <- "none"
      warning(
        "Discrepancy: cols_value is FALSE, while dendrogram is `",
        dendrogram,
  ...
  else if (is.integer(row_value)) {
    hcr <- hclustfun(distfun(x))
    ddr <- as.dendrogram(hcr)
    ddr <- reorder(ddr, row_value)
    rowInd <- order.dendrogram(ddr)
    if (nr != length(rowInd))
      stop("row dendrogram ordering gave index of wrong length")
  }
  else if (isTRUE(row_value)) {
    row_value <- rowMeans(x, na.rm = na.rm)
    hcr <- hclustfun(distfun(x))
    ddr <- as.dendrogram(hcr)
    ddr <- reorder(ddr, row_value)
    rowInd <- order.dendrogram(ddr)
  ...
  else if (identical(cols_value, "row_value")) {
    if (nr != nc)
      stop("cols_value = \"row_value\" but nrow(x) != ncol(x)")
    if (exists("ddr")) {
      ddc <- ddr
      colInd <- order.dendrogram(ddc)
    }
  ...
  else if (is.integer(cols_value)) {
    hcc <- hclustfun(distfun(if (symm)
      x
      else
        t(x)))
    ddc <- as.dendrogram(hcc)
    ddc <- reorder(ddc, cols_value)
    colInd <- order.dendrogram(ddc)
    if (nc != length(colInd))
      stop("column dendrogram ordering gave index of wrong length")
  }
  else if (isTRUE(cols_value)) {
    cols_value <- colMeans(x, na.rm = na.rm)
    hcc <- hclustfun(distfun(if (symm)
      x
      else
        t(x)))
